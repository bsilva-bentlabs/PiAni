...
class PiAni :
    ...
    def __init__(self, servo_def):
        ...
        #arrays to hold servo positions
        self.servo_def = servo_def
        self.max_time = 500      #max length of performance in seconds.  **** Really should be dynamic.
        self.max_array = int(self.max_time / PiAni.time_slice + 1)
        ...
        # *&^%^$% This method to define an array seems logically backwards.  this defines an array of servo_pos[12001][16]
        self.servo_pos = [[0 for y in xrange(self.max_servos)] for x in xrange(self.max_array)]
        ...
        self.event_pos = [[0 for y in xrange(3)] for x in xrange(self.max_array)]
...
#        print servo, pos, logical_inc, mult, self.servo_cur_pos[servo], target_pos, actual_inc, target_time
    ...
    def sync(self):
        ...
        for t in range(len(self.servo_last_time)):
            self.servo_last_time[t] = tmax
    ...
    def pause(self, delay):
        ...
        tmax = max(self.servo_last_time)
        for t in range(len(self.servo_last_time)):
            self.servo_last_time[t] = tmax + int(delay / self.time_slice)
    ...
    def mplay(self, file, mouth):
        ...
        print("sampling rate:", framerate, "length:", frames, "samples,",
